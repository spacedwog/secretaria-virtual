...
class Server {
    ...
    setupRoutes() {
        ...
        // Endpoint para receber dados do Python
        this.app.post(RECORD_DATA_ENDPOINT, (req, res) => {
            const { id_paciente, id_medico, id_receita, code_medic, id_medic, nome_medic, tipo_medic, data_medic, dosagem, frequencia, consumo, observacao } = req.body;
            ...
            this.connection.query('CALL create_medic_recip(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [code_medic, id_paciente, id_medico, data_medic, observacao, id_receita, id_medic, nome_medic, tipo_medic, dosagem, frequencia, consumo, observacao]);
            this.connection.query('CALL visit_doctor(?, ?, ?, ?)', [date, time, id_paciente, id_medico]);
            res.status(200).json({ message: 'Dados recebidos com sucesso!' });
        });
        // Endpoint para receber dados do Python
        this.app.post(SAVE_DATA_ENDPOINT, (req, res) => {
            const { id_paciente, id_medico, nome_consulta_medica, appointment_date, appointment_time, reason, status } = req.body;
            if ((id_paciente && typeof id_paciente !== 'number') ||
                (id_medico && typeof id_medico !== 'number') ||
            ...
            console.log('Dados recebidos:', { id_paciente, id_medico,
                ...
                reason, status });
            this.connection.query('CALL make_appointment(?, ?, ?, ?, ?, ?, ?)', [appointment_date, appointment_time, reason, status, id_paciente, id_medico, nome_consulta_medica]);
            res.status(200).json({ message: 'Dados recebidos com sucesso!' });
        });
        this.initialize();
    }
...
function runPowerShellScriptInThread(scriptPath, params) {
    if (worker_threads_1.isMainThread) {
        ...
        const { scriptPath, params } = worker_threads_1.workerData;
